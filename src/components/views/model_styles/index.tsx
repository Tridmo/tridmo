...
function Style() {
  ...
  const handleChange = (event: React.ChangeEvent<HTMLInputElement>, id: string) => {
    ...
    if (getModelCategoryNameFilter) {
      router.push(`/?page=${getModelPageFilter}&styles=${res}&category_name=${getModelCategoryNameFilter}&category=${getModelCategoryFilter}`);
    } else {
      router.push(`/?page=${getModelPageFilter}&styles=${res}&category=${getModelCategoryFilter}`);
    }
