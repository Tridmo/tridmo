...
const Categories = () => {
  ...
  console.log(getModelCategoryFilter, "getModelCategoryFilter")
  const handleCloseAccordion = (item: any) => (event: React.SyntheticEvent, isExpanded: boolean) => {
    ...
    if (isExpanded) {
      ...
      console.log(item?.name, "iphone senga pro makis")
      dispatch(setCategoryNameFilter({ knnex: item?.name }))
      router.push(`/?page=${getModelPageFilter}&styles=${getModelStyleFilter}&category_name=${item.name}&category=${getModelCategoryFilter}`);
    ...
    if (!isExpanded) {
      ...
      router.push(`/?page=${getModelPageFilter}&styles=${getModelStyleFilter}`);
    }
  };
  ...
  useMemo(() => {
    const setChildrenData = () => {
      ...
      let arr = new Array();
      res?.children?.forEach((category: categoryProps) => {
        if (getModelCategoryFilter == category?.id || getModelCategoryFilter?.includes((category?.id)) || getModelCategoryFilter?.includes((category.id)?.toString())) {
          arr.push({
            id: category?.id,
            name: category?.name,
            parent_id: category?.parent_id,
  ...
  const handleChange = (event: React.ChangeEvent<HTMLInputElement>, id: string) => {
    ...
    router.push(`/?page=${getModelPageFilter}&styles=${getModelStyleFilter}&category_name=${getModelCategoryNameFilter}&category=${res}`);
