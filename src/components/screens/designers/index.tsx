...
export default function DesignersPage() {
  ...
  const getDesignersStatus = useSelector(
    (state: any) => state?.get_all_designers?.status
  );
  const getProfileStatus = useSelector(
    (state: any) => state?.get_profile?.status
  );
  const getDesignersNameFilter = useSelector(
    (state: any) => state?.handle_filters?.designers_name
  );
  const getDesignersPageFilter = useSelector(
    (state: any) => state?.handle_filters?.designers_page
  );
  ...
  return (
    <Box sx={{ ...ContainerStyle, mt: "32px" }}>
      {!searchValue && (
        <SimpleTypography text="Дизайнеры" className="section__title" />
      )}
      {getDesignersStatus == "succeeded" ? (
        <>
            ...
            >
              {searchValue ? (
                ...
                >
                  ...
                  <SimpleTypography
                    text={`найдено ${all__designers?.data?.pagination?.data_count} результатов`}
              ...
              <SimpleTypography
                text={`Показаны ${
                  ...
                  all__designers?.data?.pagination?.data_count ?? 0
                } дизайнеров`}
          ...
          <List sx={listSx}>
            ...
            {all__designers?.data?.designers &&
            all__designers?.data?.designers?.length != 0 ? (
              all__designers?.data?.designers?.map((user, index: any) => (
                <Link
                  key={user?.id + index}
                  href={
                    profile?.user_id == user.id && isAuthenticated
                      ? "/profile"
                      : `/designers/${user?.username}`
                  }
                >
                  <ListItem key={user?.id} alignItems="center" sx={liSx}>
                    <ListItemText
                      sx={{
                        marginRight: {
                          lg: "16px",
                    ...
                    >
                      <ListItemAvatar sx={liAvatarWrapper}>
                        <Avatar
                          src={
                            user?.image_src
                              ? `${IMAGES_BASE_URL}/${user?.image_src}`
                              : "/img/avatar.png"
                          }
                          alt=""
                          sx={liAvatarSx}
                  ...
                  {all__designers?.data?.designers?.length &&
                  index != all__designers?.data?.designers?.length - 1 ? (
                    <Divider
                      sx={{ margin: 0 }}
                      variant="inset"
                      component="li"
        ...
        <List sx={{ ...listSx, marginBottom: "32px" }}>
          <ListItem alignItems="center" key={-1} sx={liHeaderSx}>
            <SimpleTypography
              ...
              sx={{
                ...liHeaderTextSx,
                marginRight: { lg: "16px", md: "16px", sm: "16px", xs: "8px" },
                ...widthControl,
              }}
            />
            <SimpleTypography
          ...
          {fakeDesigners?.map((i) => (
            <Box key={i}>
              <ListItem key={i} alignItems="center" sx={liSx}>
                ...
                >
                  <ListItemAvatar sx={liAvatarWrapper}>
                    <Skeleton variant="rounded" sx={liAvatarSx} />
                  </ListItemAvatar>
