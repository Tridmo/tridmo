...
export default function OneModel() {
  ...
  React.useMemo(() => {
    if (similarModelsData) {
      setSimilarModels((prev) => {
        ...
        const newModels = similarModelsData?.data?.models?.filter(
          (m) => !existingIds.has(m?.id) && m?.id != model?.id
        );
  ...
  return (
    <Box sx={ContainerStyle}>
      ...
      {/* INTERIORS */}
      {model?.used_interiors?.length > 0 &&
        model?.used_interiors[0] != null && (
          <>
            <Box
              sx={{
                display: "flex",
            ...
            >
              {model?.used_interiors?.map((interior: any, index: any) => (
                <Grid
                  className="models__card"
                  key={interior?.interior?.id + index}
                  md={3}
                  sm={4}
                  xs={xxsScreen ? 12 : 6}
                  item
