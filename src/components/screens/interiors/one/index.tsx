...
import { Avatar, Box, Button, Divider, Grid, ListItemAvatar, Menu, MenuItem, Paper, styled, useMediaQuery } from '@mui/material';
...
import { VisibilityOutlined, VisibilityOffOutlined, Favorite, FavoriteBorder, Sell, Visibility, SellOutlined, LibraryAdd, LibraryAddOutlined } from '@mui/icons-material';
...
export default function OneInterior() {
  ...
  const smScreen = useMediaQuery('(max-width:496px)');
  const mdScreen = useMediaQuery('(max-width:846px)');
  ...
  return (
    <Box sx={ContainerStyle}>
      ...
      >
        ...
        <Grid item
          lg={isAuthenticated && interior?.author?.id == currentUser?.user_id ? 3 : 5}
          md={isAuthenticated && interior?.author?.id == currentUser?.user_id ? 3 : 5}
          sm={12} xs={12}
          sx={{
            display: 'flex',
            alignItems: 'center',
        ...
        <Grid item
          lg={isAuthenticated && interior?.author?.id == currentUser?.user_id ? 9 : 7}
          md={isAuthenticated && interior?.author?.id == currentUser?.user_id ? 9 : 7}
          sm={12} xs={12}
          sx={{
        ...
        >
          ...
          <Box margin={'0 0 0 16px'}>
            <Buttons
              name={mdScreen ? '' : `${showTags ? 'Скрыть' : 'Показать'} бирки`}
              onClick={setShowTags}
            ...
            >
              {
                mdScreen ? (
                  showTags ? <Sell sx={{ color: primaryColor }} /> : <SellOutlined />
                ) : (
                  showTags ? <VisibilityOffOutlined sx={{ mr: '8px' }} /> : <VisibilityOutlined sx={{ mr: '8px' }} />
                )
              }
            </Buttons>
          </Box>
          {
            isAuthenticated && interior?.author?.id === currentUser?.user_id
              ...
              <>
                ...
                <Box margin={'0 0 0 16px'}>
                  <Buttons
                    name={mdScreen ? `${likesCount}` : `Нравиться  (${likesCount})`}
                    className='bookmark__btn'
                    childrenFirst={true}
                    onClick={handleLike}
                  >
                    {isLiked ? <Favorite sx={{ mr: '8px', color: primaryColor }} /> : <FavoriteBorder sx={{ mr: '8px' }} />}
                  </Buttons>
                </Box>
                <Box margin={'0 0 0 16px'}>
                  <Buttons
